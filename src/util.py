...
def check_data_format(tweets, labels=None):
    """
    Check that the data provided is list of string and labels a list (and normalize if numerical labels)
    :param tweets: list data/tweets
    ...
    and bool(labels) and isinstance(labels, list)):
        raise ValueError('Incorrect format of tweets and labels. Must be list of strings and list of strings or numbers')
    if isinstance(labels[0], str):
        if labels[0].strip("-+").isdigit():
            try:
                labels[:] = list(map(int, labels))
            except ValueError:
                raise ValueError('The labels must be either all string or all numbers.')
    if not len(set(labels)) in number_classes:
        raise ValueError('The module only accepts 2/3/5 classes data')
    return True

def check_compatibility_train_test(train_labels, test_labels):
    """
    Check that train and test are comparable datasets (labels numbers, type...).
    Assumes that check_data_type has been called before.
    :param train_labels: train labels
    :param test_labels: test labels
    :return: True or raises Exception
    """
    if all(isinstance(item, int) for item in train_labels):
        if not all(isinstance(item, int) for item in test_labels):
            raise ValueError('Train and test labels must be both either string or int!')
    elif all(isinstance(item, str) for item in train_labels):
        if not all(isinstance(item, str) for item in test_labels):
            raise ValueError('Train and test labels must be both either string or int!')
    # if not set(train_labels) == set(test_labels):
    #     raise ValueError('Train and test data are not compatible !')
    else:
        return True
...
def convert_to_dataframe(X, y=None, columns=None):
    ...
    if columns and not isinstance(columns, list):
        raise ValueError('The columns must be in a list')
    if issparse(X):
...
def macro_MAE(y_true, y_predicted, classes=[-2, -1, 0, 1, 2]):
    ...
    :return: MAE
    """
    if not all(isinstance(item, int) for item in y_true) or not all(isinstance(item, int) for item in y_predicted):
