...
def doc2vec_train(tweets, labels, min_count=1, window=10, size=100, sample=1e-4, negative=5, workers=7, epochs=10):
    ...
    try:
        model = Doc2Vec.load('doc2vec.pkl')
    except FileNotFoundError:
        print("No model saved. Training a new model on train data, and save to doc2vec.pkl")
        tweets, labels = process_for_doc2vec(tweets, labels)
        ...
        try:
            model.save('doc2vec.pkl')
        except (pickle.PicklingError, FileNotFoundError) as e:
            print("Model trained but could not be saved")
    return model
